...
public class RestData {
	...
	public String getTriangle(@PathParam(value = "a") int a, @PathParam(value = "b") int b,
		...
		if (a <= 0 || b <= 0 || c <= 0) {
			throw new BadRequest("enter valied details");
		}
	...
	public Map<String, Integer[]> makeOneArray(@RequestBody Map<String, Integer[]> array, HttpServletResponse response)
		...
		for (Integer[] value : array.values()) {
			for (Integer single : value) {
				if (single.getClass().isInstance(new String())) {
					throw new BadRequest("enter integer values only!!");
				}
			}
		}
